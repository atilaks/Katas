...
class Proxy:
    ...
    def power (self):
        ...
        self.add_messages("power")
        self.set_on(True)
  
    def is_on (self):
        return self._is_on

    def add_messages(self, text):
        self.messages_list.append(text)

    def messages(self):
        return self.messages_list
        
    def was_called(self, input):
        return input in self.messages_list

    def number_of_times_called(self, input):
        counter = 0
        for i in range(len(self.messages_list)):
            if input in self.messages_list[i]:
                counter += 1
        return counter
